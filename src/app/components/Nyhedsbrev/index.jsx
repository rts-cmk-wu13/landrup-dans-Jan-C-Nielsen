    ...
    return (
        <section className="text-white m-[27] flex flex-col  sm:items-start sm:text-left">
            ...
            <form className = " text-white flex mx-auto" action={formAction} noValidate>
                   <div>
                    <div>
                        <input  className = "bg-white  mr-[27px]" type="email" name="email" defaultValue={state.values.email}></input>
                        {state.errors?.email && <p>{state.errors.email}</p>}
                    </div>

                </div>
                {state.errors?.form && <p>{state.errors.form}</p>}
                <button  type="submit" disabled={isPending} className="bg-white p-[11px] text-black rounded-[10px] disabled:bg-grey-400">{isPending ? "Opretter..." : "Log ind"}</button> 
          </form>
        </section>
    )
